...
function setup (config) {
  ...
  function page (src) {
    console.log("Paging " + src);
    var header = parse.header(src);
    console.log("Header was " + header);
    var compiled_src = apply_compilers(parse.content(src));
    console.log("Compiled content was " + compiled_src)
    console.log(bundle(compiled_src, header).toString());
    return bundle(compiled_src, header);
  }

  function apply_compilers (src) {
    var result = src
    compilers.forEach(function(compiler) {
      console.log("Applying compiler: " + compiler.compile);
      result = compiler.compile(result);
    });
    return result;
  };

  function bundle (str, parameters) {
    console.log("Bundling " + str + " and " + parameters);
