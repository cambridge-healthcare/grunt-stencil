...
function setup (separator) {
  return {
    parsed: with_chunks(parsed),
    header: with_chunks(header),
    content: with_chunks(content)
  };

  function parsed (chunks) {
    var header_result = header(chunks);
    var content_result = content(chunks);

    return _.extend(header_result,
                    { toString: function () { return content_result; } });
  }
