...
function setup (config) {
  ...
  var initial_template_vars = _.extend({}, options.vars, { include: process_partial });
  ...
  function page (input_file, params) {
    ...
    var template_vars = bundle_template_vars(header, params);
    var content = compile(input_file, template_vars);

    if (header.template) {
      content = inject_to_template(content, header.template, template_vars);
    }

    return bundle_page(content, header);
  }

  function inject_to_template (page, template, params) {
    var template_vars = bundle_template_vars(params, {document: page});
    return process_template(template, template_vars).toString();
  }

  function bundle_template_vars () {
    return _.flatten([
