...
function prepare_it_obj (options) {

  var it = options.dot_it_object;

  // Convert given patterns to actual file paths
  it.file_lists = expand_file_lists(it.file_lists);

  // Construct an include function
  // to give it_obj access to contents of a given file
  it.include = get_partial_src.bind(null,
                                    it,
...
function get_partial_src (dot_it_obj, dot_template_settings, base_folder, rel_path) {
  console.log("Getting partial " + rel_path);
  // Find the file and get its source
  var input_file = find_closest_match(base_folder, rel_path);
  var src = grunt.file.read(input_file);

  // Compile the file's source using dot
  // (regardless of whether the file is dot)
  //var it = _.extend(dot_it_obj, { include: get_partial_src });
  //console.log(it);
  //var compiled_dot = compile_dot(_.extend(dot.templateSettings, {strip: false}),
//                                 "\n\n", input_file, it)

  // If the file has a markdown extension, compile it, too

  // Return the compiled source

  return "File is here: " + input_file + " THIS WAS A PARTIAL!!!"
}

// function include (path, it) {
//   var src = grunt.file.read(path);

//   return [function compile_dot (src) {
//     if (is_dot(path)) {
//       it = _.extend({}, it, { include: include });
//       return dot.template(src)(it);
//     } else {
//       return src;
//     }
//   }, function compile_md (src) {
//     if (is_md(path)) {
